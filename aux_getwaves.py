...
[Default: {def_format}]")
    parser.add_argument('-all', '--all', action='store_true',
                        help=f"Retrieve all the channels available (instead of first available among HH, EH, BH, SH)")
    parser.add_argument('-mg', '--merge', action='store_true',
                        help='Merge gappy data by filling the gaps (with latest value)')
...
if __name__=="__main__":
    ...
    st = _get_data(starttime, endtime, acq_dir, 
                   dict_station, components, ball=all_bool, merge=merge)
    ## Output data
